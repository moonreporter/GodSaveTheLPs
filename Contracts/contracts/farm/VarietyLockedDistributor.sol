...
contract VarietyLockedDistributor is Ownable, ReentrancyGuard {
    // remember to change for mainnet deploy
    address constant _trustedForwarder =
        0x8045427deE7219843934bC464584f287720DBEB0; //TRUSTED FORWARDER
    ...
    constructor(IVarietyERC20 _Variety, uint256 _VarietyPerBlock) {
    ...
    function pendingVariety(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 VarietyReward = multiplier
                .mul(VarietyPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accVarietyPerShare = accVarietyPerShare.add(
                VarietyReward.mul(1e12).div(lpSupply)
            );
        }

        uint256 pending = user.amount.mul(accVarietyPerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        require(
            block.number >= startBlock,
            "VarietyDistributor: Can not deposit before start"
        );
    ...
    function payOrLockupPendingVariety(uint256 _pid) internal {
        ...
        uint256 pending = user.amount.mul(pool.accVarietyPerShare).div(1e12).sub(
            user.rewardDebt
        );
